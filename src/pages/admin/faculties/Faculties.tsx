...
const Faculties = () => {
	...
	useEffect(() => {
		if (data) {
			const results = data.filter(
				(faculty: facultyCoalesced) =>
					faculty.name
						.toLowerCase()
						.includes(searchTerm.toLowerCase()) ||
					faculty.person.first_name
						.toLowerCase()
						.includes(searchTerm.toLowerCase()) ||
					faculty.person.last_name
						.toLowerCase()
						.includes(searchTerm.toLowerCase()),
			);
	...
	return (
		<Flex gap={10} flexDir={'column'}>
			...
			/>
			<SectionHeader
				addText={t('addNewFaculty') as string}
				deleteButton={false}
				onChange={handleSearch}
				onAddClick={onOpen}
				searchPlaceholder={t('searchFaculties') as string}
			/>
			{isLoading ? (
				<Flex
					w={'100%'}
							...
							<Tr key={faculty.id}>
								...
								<Td>
									<IconButton
										...
										onClick={() =>
											handleDelete(
												faculty.id as unknown as string,
											)
										}
									/>
								</Td>
