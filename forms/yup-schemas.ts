...
function peselValidation(pesel: string | undefined): boolean {
	...
	if (year >= 20) {
		...
		calculatedControlNumber =
			calculatedControlNumber === 10 ? 0 : calculatedControlNumber;
...
const personalSchema = yup.object().shape({
	...
	disabled: yup
		.boolean()
		.required('Select disability status')
...
const addressContactSchema = yup.object().shape({
	...
	country: yup
		.string()
		.matches(/^[A-Z]{2}$/, 'Country code must be exactly 2 charaters long')
		.required(),
	postal_code: yup
		.string()
		.matches(/^[0-9]{2}-[0-9]{3}$/, 'Invalid postal code'),
	street: yup.string().required(),
	email: yup.string().email().required(),
	phone_number: yup
		.string()
		.required()
		.matches(/^[0-9]{9}$/, 'Phone number must be exactly 9 digits long'),
});
